...
def _build_gemini_prompt(
    ...
    return (
        ...
        "- When an element line carries `RENDER AS:`, that role was computed deterministically from the drawing (label keywords, shape, position). It OVERRIDES your own inference — render exactly that element type. A `RENDER AS: input` card is ALWAYS an <input> with its label as the placeholder, never a <p> or heading. A `RENDER AS: container (group box)` card is a wrapper (<div>/<section>) — the components whose boxes fall inside its bounds render as its CHILDREN; never render it as a literal card or text stub. A `likely:` hint is only a shape-based default — the text drawn on/in the component (from its annotation or the attached image) wins over it.\n"

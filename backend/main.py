...
# A label-style text annotation whose only enclosing element is a container
# this big or larger (as a fraction of canvas area) is considered "orphan" —
# its real input/button rectangle was probably missed by the detector.
_ORPHAN_LABEL_CONTAINER_RATIO = 0.10

# Vocabulary used to detect orphan input labels and orphan button texts.
# Lower-case, stripped of trailing colon before lookup.
# Roboflow v2 frequently misclassifies wide-thin input rectangles as
# `section` / `footer` (similar shape) — when that happens the input is
# rejected below threshold and the user's label text has nowhere to bind.
# This vocabulary lets us synthesize a virtual input/button around the
# orphan label. Remove once v3 ships and reliably detects inner inputs.
_INPUT_LABEL_WORDS = {
    "email", "e-mail", "password", "confirm password", "old password",
    "new password", "current password", "username", "user name", "name",
    "full name", "first name", "last name", "phone", "phone number",
    "mobile", "address", "street", "city", "country", "zip", "zip code",
    "postal code", "state", "region", "company", "title", "subject",
    "message", "comment", "search", "query",
}
_BUTTON_TEXT_WORDS = {
    "submit", "send", "save", "continue", "next", "back", "cancel",
    "sign in", "signin", "log in", "login", "sign up", "signup",
    "register", "sign out", "log out", "logout",
    "ok", "okay", "yes", "no", "close", "delete", "remove", "add",
    "create", "update", "edit", "subscribe", "join", "confirm", "apply",
    "checkout", "buy now", "purchase", "get started", "learn more",
    "read more", "download", "upload", "share", "follow", "like",
}


def _looks_like_input_label(text: str) -> bool:
    """True if `text` reads like a form-field label ('Email', 'Password', etc.)."""
    t = (text or "").strip().lower()
    if not t:
        return False
    if t.endswith(":"):  # explicit "Label:" form
        return True
    return t.rstrip(":").strip() in _INPUT_LABEL_WORDS


def _looks_like_button_text(text: str) -> bool:
    """True if `text` reads like an action-verb button label ('Sign In', 'Submit')."""
    t = (text or "").strip().lower().rstrip(".").rstrip("!")
    if not t:
        return False
    return t in _BUTTON_TEXT_WORDS


def _find_enclosing_element(ann: TextAnnotation, elements: List[Any]):
    """Smallest detected element whose bbox encloses the annotation's center."""
    ...
    for elem in elements:
        ...
        try:
            ex = float(b.get("x", 0))
            ey = float(b.get("y", 0))
            ew = float(b.get("width", 0))
            eh = float(b.get("height", 0))
...
def _synthesize_inputs_for_orphan_labels(
    elements: List[Any],
    annotations: List[TextAnnotation],
    canvas_size: Optional[tuple],
    *,
    ...
    """
    Stop-gap synthesizer for input fields and buttons the detector missed.

    Roboflow v2 often misclassifies wide-thin input rectangles as `section`
    or `footer` (similar aspect ratio) and rejects them via per-class
    thresholds. The user's label text ("Email", "Password") then has nowhere
    to bind, the strict-fidelity prompt rule stops Gemini from inventing a
    missing input, and the preview renders a floating label with no input
    underneath.

    Logic: for each text annotation whose semantic content reads as an input
    label (Email / Password / Username / ...) or button action (Sign In /
    Submit / ...) AND whose only enclosing detected element is a large
    container (i.e. no small card was detected to represent the input/button
    the user actually drew), synthesize a virtual card around the label
    so the matcher binds the label to a real-sized element.

    Skip when the smallest enclosing element is already small (< 10% canvas
    area) — that means the user's rectangle WAS detected as a small card and
    no synthesis is needed.

    Remove this once Roboflow v3 (2,700 images) ships and reliably detects
    inner input/button rectangles. The whole function becomes dormant once
    the detector covers every drawn shape.
    """
    from app.models.inference import ExternalModelElement

    if not annotations or not canvas_size:
        return elements

    canvas_w, canvas_h = float(canvas_size[0]), float(canvas_size[1])
    if canvas_w <= 0 or canvas_h <= 0:
        return elements
    container_area_threshold = canvas_w * canvas_h * _ORPHAN_LABEL_CONTAINER_RATIO

    # Look for an already-detected input-shaped card to template dimensions off.
    # If the user drew one input correctly (e.g., Email) and we missed another
    # (e.g., Password), mirroring keeps the synth input visually consistent.
    sibling_x = None
    sibling_w = None
    sibling_h = None
    for elem in elements:
        if (getattr(elem, "type", "") or "").lower() != "card":
            continue
        b = getattr(elem, "bounds", None) or {}
        try:
            ex = float(b.get("x", 0))
            ...
            continue
        if ew <= 0 or eh <= 0 or eh > 120:
            continue
        if ew / eh < 3.0:  # not wide-thin → not input-shaped
            continue
        if sibling_h is None or eh < sibling_h:
            sibling_x = ex
            sibling_w = ew
            sibling_h = eh
    ...
    for ann in annotations:
        ...
        is_input = _looks_like_input_label(text)
        is_button = (not is_input) and _looks_like_button_text(text)
        if not is_input and not is_button:
            continue

        enclosing = _find_enclosing_element(ann, new_elements)
        if enclosing is None:
            continue  # floating text — handled elsewhere as unbound

        b = getattr(enclosing, "bounds", None) or {}
        try:
            ex = float(b.get("x", 0))
            ey = float(b.get("y", 0))
            ew = float(b.get("width", 0))
        ...
        # If the label is already inside a small element, the user's input/button
        # was detected — don't double up.
        if ew * eh < container_area_threshold:
            continue

        label_cx = float(ann.x) + max(float(ann.width), 1.0) / 2.0
        label_cy = float(ann.y) + max(float(ann.height), 1.0) / 2.0

        if is_input:
            # Mirror sibling input shape when available — visual consistency.
            synth_w = sibling_w if sibling_w is not None else max(ew - 40.0, ew * 0.7)
            synth_h = sibling_h if sibling_h is not None else 50.0
            synth_x = sibling_x if sibling_x is not None else (ex + (ew - synth_w) / 2.0)
            synth_y = label_cy - synth_h / 2.0
            role = "input"
        else:
            # Button: narrower than an input, centered on the label.
            synth_w = max(min(canvas_w * 0.18, 220.0), 120.0)
            synth_h = (sibling_h * 0.75) if sibling_h else 48.0
            synth_h = max(synth_h, 40.0)
            synth_x = label_cx - synth_w / 2.0
            synth_y = label_cy - synth_h / 2.0
            role = "button"

        # Clamp inside the container with a small margin so the synth element
        # stays a true child of its parent.
        synth_x = max(ex + 8.0, min(synth_x, ex + ew - 8.0 - synth_w))
        synth_y = max(ey + 8.0, min(synth_y, ey + eh - 8.0 - synth_h))

        synth_counter += 1
        synth_elem = ExternalModelElement(
            id=f"synthetic-{role}-{synth_counter}",
            type="card",
            confidence=0.0,
            label=f"{role.title()} ({text})",
            bounds={
                "x": float(synth_x),
                "y": float(synth_y),
                "width": float(synth_w),
                "height": float(synth_h),
            },
            attributes={
                "synthetic": True,
                "synthetic_role": role,
                "reason": (
                    f"detector returned no {role}-shaped card around the "
                    f"'{text}' annotation — inferred from label semantics"
                ),
            },
        )
        new_elements.append(synth_elem)
        if debug:
            print(
                f"[debug] synthesized {role} for orphan label '{text}' "
                f"at ({synth_x:.0f},{synth_y:.0f},{synth_w:.0f},{synth_h:.0f}) "
                f"(parent {enclosing.type}, area > {int(_ORPHAN_LABEL_CONTAINER_RATIO * 100)}% canvas)"
            )
...
async def resolve_external_model_output(
    ...
    # Stop-gap until Roboflow v3 lands: v2 frequently misses inner inputs and
    # buttons (misclassifies wide-thin shapes as section/footer below threshold).
    # Synthesize a virtual card around any orphan input-label / button-text
    # annotation so strict-fidelity rendering doesn't lose the element.
    roboflow_output.elements = _synthesize_inputs_for_orphan_labels(
        roboflow_output.elements,
        annotations,
        inferred_canvas_size,
        debug=debug,

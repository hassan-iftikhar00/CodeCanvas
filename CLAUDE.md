...
| LLM                        | Purpose                                                        |
| -------------------------- | -------------------------------------------------------------- |
| **Gemini 3.5 Flash**       | Primary for ALL LLM paths (free tier; beats 2.5 Pro on coding) |
| **Gemini 3 Flash Preview** | First fallback (free; serves on every pool key)                |
| **Gemini 2.5 Flash**       | Last resort (being phased off free tier for newer projects)    |

Ladder is env-driven (`GEMINI_MODELS`, comma-separated, best first) — see Decision #29. Pro models left the API free tier on 2026-04-01 (paid-only now). Code generation AND chat refinement both run on the Gemini key pool (free-key rotation across separate Google accounts). OpenRouter fully removed. `python backend/scripts/check_gemini_keys.py` health-checks every pool key against the ladder.
...
GEMINI_API_KEY=            # For code generation (slot 1, tried first — put a HEALTHY key here)
GEMINI_API_KEY_2=          # ... any _N: free-key rotation pool, one key per Google account (see inference.py)
GEMINI_MODELS=             # optional: comma-separated model ladder, best first
                           # (default: gemini-3.5-flash,gemini-3-flash-preview,gemini-2.5-flash)
...
### ✅ Fully Done
- Free Gemini strategy: model ladder + key health tester + hidden model-control panel (2026-07-17, Hassan — the paid 2.5-pro account drained $9.08/$10 in 13 days and got suspended; research found Google removed ALL Pro models from the API free tier on 2026-04-01, while free `gemini-3.5-flash` beats 2.5-pro on coding benchmarks): (a) model ladder is now env-driven — `GEMINI_MODELS` (default `gemini-3.5-flash,gemini-3-flash-preview,gemini-2.5-flash`), parsed by `_parse_gemini_models` in inference.py; rotation/cooldown machinery unchanged, works with any ladder length. (b) `generate_with_gemini` gained `force_model` (restricts the loop to one ladder member; unknown values ignored so a stale client never bricks generation); additive `forceModel` field on GenerateCodeRequest/RepairRequest/AnnotateRequest, threaded through all four LLM call sites; `_generation_cache_key` gained a model segment (`|auto` default). (c) NEW `backend/scripts/check_gemini_keys.py` — pings every `GEMINI_API_KEY*` on each ladder model via raw REST, masked-key table + `--json`; live run 2026-07-17: 8/10 keys OK on 3.5-flash, 9/10 on 3-flash-preview, only 2/10 on 2.5-flash (Google no longer serves 2.5-flash to newer free projects — that's why SAVER forces 3-flash-preview, not 2.5-flash), slot-1 paid key dead (RATE_LIMITED on all). (d) NEW `GET /api/llm-status` (via `get_llm_pool_status`) — per key×model cooldown remaining + today's success counts (`_key_model_success_counts`, UTC-day bucketed); key slots only, never key material. (e) NEW hidden `ModelControlPanel.tsx` on the canvas page (Ctrl+Shift+G or Cmd+K "Model control"): AUTO (full ladder) / BEST (force 3.5-flash, fails rather than degrades — for judges) / SAVER (force 3-flash-preview, preserves 3.5 quota) + key status grid (manual refresh) + auto-repair toggle (off = skip the extra repair Gemini call; fidelity scoring stays on, it costs Roboflow not Gemini); mode + toggle persist in localStorage and a `MODE · BEST/SAVER` micro-pill shows in the detection strip when not AUTO. 15 new backend tests (`test_model_ladder.py`, 263 total), tsc clean, 40 frontend tests pass. NOTE: `pnpm lint` had 238 PRE-EXISTING prettier errors on clean main (CRLF/prettier drift since the 07-13 commit — CI frontend job red before this work); my files lint clean, repo-wide `--fix` deferred. User .env action: put a healthy free key in `GEMINI_API_KEY` slot 1 and drop the dead paid key.
- Annotate-vs-repair race + annotate over-targeting (2026-07-13, Hassan — live test: first annotate "added a Card"; circling a button with "make this red only" painted the whole form): (1) Phantom Card was NOT annotate — the background fidelity auto-repair (post-generation self-check scored a correct borderless form < 0.8) finished AFTER the annotate and blindly `setGeneratedCode(repaired.code)`, overwriting the annotate result with a repair of the PRE-annotate code into which Gemini injected literal "Card" stubs (detector-class words exist only in the repair prompt's vocabulary — fingerprint). Fixes: staleness guard in `scoreFidelity` (badge + repair dropped when `previousGenRef.current.code !== opts.code`, checked after scoring AND after the 40-80s repair call; switched-screen branch only overwrites the snapshot if it still holds the scored code) + backend `/api/repair` second sanity guard `_repair_introduced_class_stubs` (422 when output renders a detector-class name as visible text the input didn't — the prompt rule alone was violated live). (2) Whole-form-red: target resolution kept EVERY intersecting `data-cc-id` box — a circle around a button also crosses the form wrapper + page container, and the prompt applied the note to all. `LivePreview.handleApplyAnnotation` now resolves innermost targets: coverage = markup-bbox∩element / element area; coverage ≥ 0.55 = "enclosed" (deliberately circled) and wins; else ancestors whose rect contains another matched target are dropped. Circling a whole form still targets the form (itself enclosed). Annotation prompt gained: "this"/"it" binds to marked elements only; container+children = ONE marked area. 5 new backend tests (248 total). NOTE: Email/Full Name order swap in the same test predates annotate (present in the raw generation) — not yet diagnosed.
- Multi-screen isolation audit (2026-07-12, Hassan — full re-audit of feature A after cross-screen leaks): 11 fixes across `page.tsx`, `useHistory.ts`, `SketchCanvasWithHistory.tsx`. Highest severity: (1) autosave was handing RAW `history.state` to `useAutoSave` — `updateProject` overwrites `canvas_data` wholesale, so 3s after any stroke on a 2+ screen project the `screens` array (every other screen's sketch+code) was WIPED from the projects row; autosave now saves a screens-wrapped memo payload. (2) The undo stack is shared across screens and screen switches PUSHED onto it — Ctrl+Z after a switch painted the previous screen's strokes onto the current one (then snapshot capture persisted the corruption); `useHistory` gained `reset(state)` and switches now replace the stack. (3) `useHistory` installed its own Ctrl+Z listener on top of the page's → every undo fired twice (index could step to -1); hook listener removed, page owns shortcuts, undo/redo now return the target state and apply it imperatively via new wrapper `applyState` (guard, no history write) closing the same 500ms-poll race for undo/redo. (4) Async writers (chat, annotate, fidelity badge, auto-repair) now capture `originScreenId` and route results to the origin screen's snapshot when the user switched tabs mid-flight (generation already did after the 07-10 fix); new `persistRefinedCode` helper persists refinements through committed `screensRef` data, never through stale closures. (5) HITL: origin stamped at DETECT time (`RunGenerationOpts.originScreenId`) since the review overlay can span a tab switch; `currentScreen` in the payload now uses the origin's name. (6) `?id=` loader + version restore now load the active screen THROUGH `loadScreenIntoWorkspace` (snapshot's code is authoritative — top-level `generated_code` can mirror a different screen) and use the guarded `loadState` path instead of raw `history.setState` (poll race). (7) Checkpoints store `getPersistableCanvasData()` (raw read was screen-less, and a no-op in upload mode). (8) `ensureGenerationProject` wraps the first-save payload with screens so a failed first generation can't strand a screen-less row. tsc clean, 0 lint errors, 40/40 frontend tests.
...
| `backend/scripts/check_gemini_keys.py` | Key pool health tester: pings every `GEMINI_API_KEY*` on each ladder model, masked-key table (`--json` for machine output) |
| `src/components/canvas/ModelControlPanel.tsx` | Hidden model-control panel (Ctrl+Shift+G): AUTO/BEST/SAVER mode, key status grid, auto-repair toggle |
| `src/app/api/llm-status/route.ts` | Proxy to FastAPI `GET /api/llm-status` (key pool cooldowns + success counts) |
...
29. **Free 3.x Flash ladder replaced paid 2.5 Pro; forced-model choice is a client hint, never a hard contract (2026-07-17)** — Google removed all Pro models from the Gemini API free tier on 2026-04-01, and free `gemini-3.5-flash` outscores 2.5-pro on coding/React benchmarks, so the paid key is pointless: the ladder (`GEMINI_MODELS` env, default `gemini-3.5-flash,gemini-3-flash-preview,gemini-2.5-flash`) runs entirely on free keys from SEPARATE Google accounts (free quota is per-project — same-account keys share one pool and add nothing). Do NOT add `gemini-2.5-flash`-first modes: the key health check showed Google no longer serves it to newer free projects (2/10 keys). `forceModel` from the UI panel restricts the ladder for that request but unknown/stale values silently fall back to the full ladder — a client must never be able to brick generation with a bad model string. External providers (Groq/Cerebras Qwen3-Coder, DeepSeek) were researched and rejected: they rank below 2.5-pro, which ranks below 3.x Flash — worse output for new integration surface. Run `backend/scripts/check_gemini_keys.py` before demo days; a key showing per-day cooldown in the panel is exhausted until the next UTC day.
